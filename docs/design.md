...
2. 解密函数移植 + 单元测试（离线可测，不需要上设备）← **当前阶段**
3. 最小分页渲染 + QML 阅读器 app + AppLoad 接入，先跑通"打开一本书翻页"
4. **设备原生登录 UI**——阶段 1 现在的登录方式（CLI 打印链接，人在另一台
   ...
   - 这一步是可用性的硬前提（没有它，设备离开电脑就没法首次登录），不
     是锦上添花，得跟阶段 3 的 QML app 一起做，不能拖到最后
5. 批注（复用现有手势链路——到这一步再定手势追踪代码怎么在两个仓库间
   共享）
6. 热门划线/评论（按需拉取 + REweread 的防御性节流策略）
